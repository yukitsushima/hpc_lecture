...
int main(int argc, char** argv) {
  ...
  for(int irank=0; irank<size; irank++) {
    ...
    MPI_Win win;
    //To avoid the collision of read with write, allocate different memory 
    MPI_Win_create(recv_jbody,N/size*sizeof(Body),sizeof(Body),MPI_INFO_NULL,MPI_COMM_WORLD,&win);
    MPI_Win_fence(0,win);
    MPI_Put(jbody,N/size,MPI_BODY,send_to,0,N/size,MPI_BODY,win);
    MPI_Win_fence(0,win);
    //write back
    for(int i=0;i<N/size;i++){
      jbody[i]=recv_jbody[i];
    }
